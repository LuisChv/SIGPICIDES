...
<html lang="en">
    ...
    <body>
        @if ($etapa==1)
            ...
            <a href="{{url('mis_solicitudes')}}">Evaluación</a>
        @else
            <h3>Hola {{$name}}, este email es para notificarte que la planificación de tu proyecto: "{{$nombreProyecto}}" ha sido evaluado.</h3>            
            <p>Por favor revisa la evaluación en la sección de solicitudes o dando click en el siguiente enlace: </p>
            <a href="{{url('mis_solicitudes')}}">Evaluación</a>    
        @endif                  
    </body>
